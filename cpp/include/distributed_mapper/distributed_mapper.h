...
class DistributedMapper{
  ...
  public:
    ...
    //SET use the robust kernel DCS
    void setUseDCSFlag(const bool& use_DCS, const double& DCS_phir, const double& DCS_phip){
        use_DCS_ = use_DCS;
        DCS_phir_ = DCS_phir;
        DCS_phip_ = DCS_phip;
    }
...
/**
 * @brief used for the second stage DCS poseEstimation, update the OriginNeighbor_
 */
gtsam::Pose3 originInitialPosesAt(const gtsam::Key& key){return originInitial_.at<gtsam::Pose3>(key);}

/**
 * @brief update originNeighbor_ after the first stage complete
 */
void updateOriginNeighbor(gtsam::Key& key, const gtsam::Pose3& pose){
        if(originNeighbor_.exists(key)){
            originNeighbor_.update(key, pose);
        }
        else {
            originNeighbor_.insert(key, pose);
        }
    }
/**
 * @brief covert linear rotation to poses with initial translation
 */
void covertLinRotToPosesWithInitialTranslation(){
        gtsam::Values rotValue = gtsam::InitializePose3::normalizeRelaxedRotations(linearized_rotation_);
        evaluation_utils::pose3WithOriginTranslation(rotValue, originInitial_);//used for the second stage pose DCS
    }
  ...
  protected:
    ...
    double DCS_phir_;
    double DCS_phip_;
    void reweightDCS4rot(gtsam::Matrix& A, gtsam::Vector& b, gtsam::Vector& errorV);
    void reweightDCS4pose(gtsam::Matrix& A, gtsam::Vector& b, gtsam::Vector& errorV);


    gtsam::Values originInitial_;
    gtsam::Values originNeighbor_;//used for the second stage pose DCS****************************************************
