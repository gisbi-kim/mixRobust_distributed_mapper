...
distributedOptimizer(std::vector< boost::shared_ptr<DistributedMapper> >& dist_mappers,
  ...
  if (use_pcm && contains_odometry) {
    max_clique_size = distributed_pcm::DistributedPCM::solveCentralized(dist_mappers, graph_and_values_vec.get(),
                                                             confidence_probability, use_covariance, use_pcmHeu);
  }
  ...
  for(size_t robot = 0; robot < nr_robots; robot++){
    dist_mappers[robot]->covertLinRotToPosesWithInitialTranslation();
  ...
  for(size_t robot = 0; robot < nr_robots; robot++){
    ...
    for(const gtsam::Values::ConstKeyValuePair& key_value: neighbors){
      ...
      //used for DCS the second stage
      char symbol = gtsam::symbolChr(key);
      if(use_landmarks) symbol = tolower(symbol);
      size_t neighbor_robot_id = robot_names.find(symbol);
      if (neighbor_robot_id != std::string::npos){

        gtsam::Pose3 pose_oriNeighbor = dist_mappers[neighbor_robot_id]->originInitialPosesAt(key);
        dist_mappers[robot]->updateOriginNeighbor(key, pose_oriNeighbor);
