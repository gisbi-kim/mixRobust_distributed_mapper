...
DistributedMapper::loadSubgraphAndCreateSubgraphEdge(const GraphAndValues& graph_and_values){
  ...
  initial_ = *(graph_and_values.second);
        originInitial_ = initial_;//used for DCS
...
//*****************************************************************************
void
DistributedMapper::reweightDCS4rot(gtsam::Matrix &A, gtsam::Vector &b, gtsam::Vector &errorV) {

      const double error = errorV.norm();
      const double e2 = error*error;
      double w;
      if (e2 > DCS_phir_)
      {
         w = 2.0 * DCS_phir_/(DCS_phir_ + e2);
      }else{
         w = 1.0;
      }
      A *= w;
...
//*****************************************************************************
    void
    DistributedMapper::reweightDCS4pose(gtsam::Matrix &A, gtsam::Vector &b, gtsam::Vector &errorV) {
  ...
  for(size_t s = 0 ; s < separator_edge_ids_.size(); s++){ // for each separator
    ...
    if(robot0 == robotName_){ // robot i owns the first key
      ...
      if(!use_flagged_init_ || neighboring_robots_initialized_[robot0]){ // if use flagged initialization and robot sharing the edge is already optimized
        ...
        Matrix A = I9;
        if(use_DCS_){
          Vector r1 = linearized_rotation_.at(key1);
          Vector errorV = r1 - M9_r0;
          reweightDCS4rot(A, M9_r0, errorV);
        }
        rot_subgraph.add(key1, A, M9_r0, model);
      }
    }
    else{
