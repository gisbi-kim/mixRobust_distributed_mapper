function [ robot_poses, robot_measurements, robot_edges_id, poses, measurements, edges_id, trajectory_size] = covert2Dto3DTORO( input_file, robots_offsets )
 nodeId = 0;
 edgeId = 0;
%% 读取2维SE2数据
...
tline = fgets(fid_g2o);
  while ischar(tline)
     foundNode = ( isempty(strfind(tline, 'VERTEX2'))==0 );
    %% 读取顶点
     if foundNode==1
         nodeId = nodeId+1;
       ...
       foundEdge = ( isempty(strfind(tline, 'EDGE2'))==0 );
     %% EDGE line
     if foundEdge ==1
         edgeId = edgeId + 1;
